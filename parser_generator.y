...
program:
        protodecs classdecs stm         { /*fprintf(stderr, "%d: program\n", call_no++);*/ char *prg; prg = initstr(prg); prg = ccstr(prg, "(program ("); prg = ccstr(prg, $1); prg = ccstr(prg, ") ("); prg = ccstr(prg, $2); prg = ccstr(prg, ") "); prg = ccstr(prg, $3); $$ = strdup(ccstr(prg, ")")); /*free(prg);*/ puts($$); return 1; }
       ;
protodec:
...
factor:                                                                                                                                                                                                                                                 
       ...
       |LAMBDA LPAR formals RPAR block factorrest               { /*fprintf(stderr, "%d: factor7\n", call_no++);*/ char *fac; fac = initstr(fac); char *p1 = extparts($6, 0); char *p2 = extparts($6, 1); fac = ccstr(fac, p1); fac = ccstr(fac, "(lambda ("); fac = ccstr(fac, $3); fac = ccstr(fac, ")"); fac = ccstr(fac, "(void)"); fac = ccstr(fac, $5); fac = ccstr(fac, ")"); $$ = strdup(ccstr(fac, p2)); }
       |LAMBDA LPAR formals RPAR COLON rtype block factorrest           { /*fprintf(stderr, "%d: factor8\n", call_no++);*/ char *fac; fac = initstr(fac); char *p1 = extparts($8, 0); char *p2 = extparts($8, 1); fac = ccstr(fac, p1); fac = ccstr(fac, "(lambda ("); fac = ccstr(fac, $3); fac = ccstr(fac, ")"); fac = ccstr(fac, $6); fac = ccstr(fac, " "); fac = ccstr(fac, $7); fac = ccstr(fac, ")"); $$ = strdup(ccstr(fac, p2)); }
